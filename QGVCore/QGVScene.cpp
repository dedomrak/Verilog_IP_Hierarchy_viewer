...
{
    Agnode_t *node = agnode(_graph->graph(), NULL,1);
    if(node == NULL)
    {
    ...
    QGVNode *item = new QGVNode(new QGVNodePrivate(node, _graph->graph()/*,pLabel*/), this);
    item->setLabel(label);
    addItem(item);
    _nodes.append(item);
    return item;
...
{
    Agedge_t* edge = agedge(_graph->graph(), source->_node->node(), target->_node->node(), NULL,1);
    if(edge == NULL)
    {
...
QGVSubGraph *QGVScene::addSubGraph(const QString &name, bool cluster)
{
    Agraph_t* sgraph;
    if(cluster)
        sgraph = agsubg(_graph->graph(), ("cluster_" + name).toLocal8Bit().data(),1);
    else
        sgraph = agsubg(_graph->graph(), name.toLocal8Bit().data(),1);
